import pytest
from todoist_module.todoist_module import on_connect, on_message
...
def test_on_connect():
    ...
    assert client.subscribe.called_once_with("mcp/commands/add_task")
    assert client.subscribe.called_once_with("mcp/commands/list_tasks")
...
def test_on_message():
    ...
    assert client.publish.called_once_with("mcp/results/add_task", Mock())
...
def test_on_message_missing_correlation_id():
    # Mock MQTT client, userdata, and message
    client = Mock()
    userdata = None
    msg = Mock()
